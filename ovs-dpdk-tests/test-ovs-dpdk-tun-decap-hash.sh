#!/bin/bash
#
# Test OVS with vxlan traffic with explicit tunnel and dp-hash after decap
#
...
function config() {
    ...
    ovs-ofctl del-groups $BRIDGE
    ovs-ofctl add-group $BRIDGE group_id=1,type=select,bucket=output:$IB_PF0_PORT0
    ...
    ovs-ofctl add-flow $BRIDGE "in_port=$PF,tun_eth_dst=$NIC_MAC,tun_type=vxlan,actions=tun_decap($VXLAN_DEV),group=1"
