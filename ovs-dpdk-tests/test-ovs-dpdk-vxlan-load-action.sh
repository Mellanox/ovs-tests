...
function add_openflow_rules() {
    ...
    ovs-ofctl add-flow br-int "table=0, idle_age=0, priority=200,tun_id=$TUNNNEL_ID,in_port=vxlan_br-int, actions=load:0x1->OXM_OF_METADATA[],resubmit(,100)"
    ovs-ofctl add-flow br-int "table=0, priority=100,idle_age=0,in_port=$IB_PF0_PORT0, actions=load:0x4f6->NXM_NX_REG6[],load:0x1->OXM_OF_METADATA[],load:0->OXM_OF_IN_PORT[],resubmit(,0)"
    ...
    ovs-ofctl add-flow br-int "table=75, priority=100,idle_age=0,reg7=0x5b2, actions=load:0->OXM_OF_IN_PORT[],load:0xaca80102->NXM_NX_TUN_IPV4_DST[],load:0x64->NXM_NX_TUN_ID[],output:vxlan_br-int"
    ...
    ovs-ofctl add-flow br-int "table=115, idle_age=0, priority=100,reg7=0x4f6, actions=output:$IB_PF0_PORT0"
