...
function add_openflow_rules() {
    ...
    ovs-ofctl add-flow br-phy "table=0,in_port=$REP2,tcp,ct_state=-trk actions=ct(zone=2, table=1)"
    ovs-ofctl add-flow br-phy "table=1,in_port=$REP2,tcp,ct_state=+trk+new actions=ct(zone=2, commit, nat(dst=${LOCAL_IP}:5201)),$REP"
    ovs-ofctl add-flow br-phy "table=1,in_port=$REP2,tcp,ct_state=+trk+est actions=ct(zone=2, nat),$REP"
    ovs-ofctl add-flow br-phy "table=0,in_port=$REP,tcp,ct_state=-trk actions=ct(zone=2, table=1)"
    ovs-ofctl add-flow br-phy "table=1,in_port=$REP,tcp,ct_state=+trk+new actions=ct(zone=2, commit, nat),$REP2"
    ovs-ofctl add-flow br-phy "table=1,in_port=$REP,tcp,ct_state=+trk+est actions=ct(zone=2, nat),$REP2"
