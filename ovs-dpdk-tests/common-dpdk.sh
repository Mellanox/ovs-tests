...
function set_ovs_dpdk_debug_logs() {
    ...
    if is_doca; then
        ovs_set_log_levels dpif_netdev:file:DBG netdev_offload_dpdk:file:DBG ovs_doca:file:DBG dpdk_offload_doca:file:DBG \
                           dpif_netdev_ext:file:DBG netdev_offload_dpdk_ext:file:DBG doca_pipe_group:file:DBG \
                           netdev_dpdk:file:DBG netdev_dpdk_ext:file:DBG
    elif is_dpdk; then
...
function __setup_common_dpdk() {
    ...
    add_expected_simx_error_for_issue 3792891 "ERR-CMD .*mlx5_cmdif_flow_table.c.*DEALLOC_FLOW_COUNTER: resource number .* is busy"
