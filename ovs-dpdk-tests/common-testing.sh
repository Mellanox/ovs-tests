...
function ovs_check_tcpdump() {
    ...
    if [ $pkts -gt $expected ]; then
        err "Expected at most $expected packets to pass but received $pkts passed."
...
function initiate_roce_traffic() {
    ...
    if is_simx && [ $time -eq 5 ]; then
        time=15
        warn "In SimX, default time is set to $time"
    fi
...
function __cleanup() {
    ...
    ip a flush dev $VF &>/dev/null
    ip a flush dev $VF2 &>/dev/null
    ip -all netns delete
