...
Command completion arrived after timeout|Error cqe|failed reclaiming pages"
    ...
    local ovs_errs="Segmentation fault|core dumped|vswitchd.*killed|ovs-vswitchd.*\|ERR\||ovs-vswitchd.*\|EMER\|"
...
function unload_modules() {
    ...
    if [ "$USE_OPENIBD" == "1" -a -e /etc/init.d/openibd ]; then
        /etc/init.d/openibd force-stop || fail "Failed to stop openibd"
    else
...
function load_modules() {
    log "Load modules"
    if [ "$USE_OPENIBD" == "1" -a -e /etc/init.d/openibd ]; then
        /etc/init.d/openibd force-start || fail "Failed to start openibd"
    else
...
RM_STATUS_LIST_CLOSED="$RM_STATUS_CLOSED $RM_STATUS_REJECTED
