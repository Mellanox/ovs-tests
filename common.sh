...
function config_sriov() {
    local num=$1
    local nic=${2:-$NIC}
    echo $num > /sys/class/net/$nic/device/sriov_numvfs || fail "Failed to config sriov_numvfs on $nic"
}
...
function check_for_errors_log() {
    ...
    local rc=0
    local sec=`get_test_time_elapsed`
    local look="health compromised|firmware internal error|assert_var|DEADLOCK|possible circular locking|WARNING:|RIP:|BUG:|refcount > 1"
