...
function config_ovs_nat() {
    ...
    ovs-ofctl1 add-flow ovs-br -O openflow13 "table=1,in_port=$VM1_PORT,ip,udp,ct_state=+trk+new,ct_zone=1,action=ct(commit,zone=1,nat(src=$NAT_IP:$NAT_PORT)),$VM2_PORT"
