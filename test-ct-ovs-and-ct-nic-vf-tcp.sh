#!/bin/bash
#
# Testing CT NIC mode with hairpin over VFs with switchdev mode and ovs CT
#
...
function cleanup() {
    ...
    ovs-vsctl del-br br-ovs1 &>/dev/null
    ovs-vsctl del-br br-ovs2 &>/dev/null
}
...
function add_openflow_rules() {
    ovs-ofctl del-flows br-ovs1
    ovs-ofctl add-flow br-ovs1 arp,actions=normal
    ovs-ofctl add-flow br-ovs1 icmp,actions=normal
    ovs-ofctl add-flow br-ovs1 "table=0, in_port=$REP,ip,ct_state=-trk actions=ct(table=1)"
    ovs-ofctl add-flow br-ovs1 "table=1, in_port=$REP,ip,ct_state=+trk+est actions=normal"
    ovs-ofctl add-flow br-ovs1 "table=1, in_port=$NIC,ip,ct_state=+trk+new actions=ct(commit),normal"

    ovs-ofctl add-flow br-ovs1 "table=0, in_port=$NIC,ip,ct_state=-trk actions=ct(table=1)"
    ovs-ofctl add-flow br-ovs1 "table=1, in_port=$NIC,ip,ct_state=+trk+est actions=normal"
...
config_ovs() {
    ...
    REP2=`get_rep 0 $NIC2`
    ovs-vsctl add-br br-ovs1
    ovs-vsctl add-port br-ovs1 $REP
    ovs-vsctl add-port br-ovs1 $NIC
    ovs-vsctl add-br br-ovs2
    ovs-vsctl add-port br-ovs2 $REP2
    ovs-vsctl add-port br-ovs2 $NIC2
