#!/bin/bash
#
# Test police jump over pedit action
# tc pedit translates to multiple flow_offload items
# Thus, it is a special end-case where single tc action jump translates
# to multiple flow_action jumps
...
function config_police() {
    ...
    tc action flush action police
    tc action add police rate ${RATE}mbit burst 1m conform-exceed pipe / jump 3
