#!/bin/bash
#
# Test ipv6 CT fwd with tcp traffic
#
...
function run() {
    ...
    echo "add ct rules"
    tc_filter add dev $REP ingress protocol ipv6 prio 2 flower $tc_verbose \
        dst_mac $mac2 ct_state -trk \
        action ct action goto chain 1

    tc_filter add dev $REP ingress protocol ipv6 chain 1 prio 2 flower $tc_verbose \
    ...
    tc_filter add dev $REP2 ingress protocol ipv6 prio 2 flower $tc_verbose \
        dst_mac $mac1 \
        action ct action goto chain 1

    tc_filter add dev $REP2 ingress protocol ipv6 prio 2 chain 1 flower $flag $tc_verbose \
