#!/bin/bash
#
# Test OVS CT tcp to remote server with ct_clear, header rewrite and snat
#
# [Kernel Upstream] Bug SW #2980771: Use case 9a/9b - pod2service - BW 0 , no trafiic 
#
...
function add_openflow_rules() {
    ...
    ovs-ofctl add-flow br-ovs "table=0, in_port=$REP,ip, actions=ct(commit,table=1,zone=64001)"
    ovs-ofctl add-flow br-ovs "table=1, in_port=$REP,ip,ct_state=+trk+est actions=ct_clear,mod_dl_dst=$mac2,ct(table=2,zone=2,nat)"
    ovs-ofctl add-flow br-ovs "table=1, in_port=$REP,ip,ct_state=+trk-est actions=ct_clear,mod_dl_dst=$mac2,ct(table=2,zone=2,nat)"
    ovs-ofctl add-flow br-ovs "table=2, in_port=$REP,ip,ct_state=+trk+new actions=ct(commit,zone=2,nat(src=$IP)),$NIC"
    ovs-ofctl add-flow br-ovs "table=2, in_port=$REP,ip,ct_state=+trk+est actions=$NIC"

    ovs-ofctl add-flow br-ovs "table=0, in_port=$NIC,ip,ct_state=-trk actions=ct(table=3,zone=2,nat)"
    ovs-ofctl add-flow br-ovs "table=3, in_port=$NIC,ip,ct_state=+trk+est actions=ct_clear,mod_dl_src=$mac3,ct(table=4,zone=64001)"
    ovs-ofctl add-flow br-ovs "table=4, in_port=$NIC,ip,ct_state=+trk+est actions=$REP"
...
function run() {
    ...
    ip netns exec ns0 ip n r $REMOTE dev $VF lladdr $mac3
