...
function run() {
    ...
    tc_filter add dev $REP ingress protocol ip chain 2 prio 2 flower $flag $flag2 $tc_verbose \
        dst_mac $mac2 \
    ...
    tc_filter add dev $REP2 ingress protocol ip prio 2 flower $flag $tc_verbose \
        dst_mac $mac1 \
        action ct zone 3 action goto chain 1

    tc_filter add dev $REP2 ingress protocol ip prio 2 chain 1 flower $flag $flag2 $tc_verbose \
        dst_mac $mac1 ct_state +trk+est \
        action ct zone 1 nat goto chain 2

    tc_filter add dev $REP2 ingress protocol ip prio 2 chain 2 flower $flag $flag2 $tc_verbose \
        dst_mac $mac1 ct_state +trk+est \
    ...
    ip netns exec ns1 timeout $((t+7)) iperf -s -t $t &
    sleep 2
