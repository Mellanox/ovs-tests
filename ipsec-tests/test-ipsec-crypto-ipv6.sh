...
function run_test() {
    ...
    clean_up $mtu
    test_tx_off_rx transport 128 ipv6 tcp
    clean_up $mtu
    test_tx_rx_off transport 128 ipv6 tcp
    clean_up $mtu
    test_tx_off_rx_off transport 128 ipv6 tcp
    clean_up $mtu

    title "transport ipv6 with key length 256 MTU $mtu"

    clean_up $mtu
    test_tx_off_rx transport 256 ipv6 tcp
    clean_up $mtu
    test_tx_rx_off transport 256 ipv6 tcp
    clean_up $mtu
    test_tx_off_rx_off transport 256 ipv6 tcp
}
