...
function clean_up(){
    clean_up_crypto 1500 over_trusted_vfs
    ipsec_cleanup_trusted_vfs_on_both_sides
}


function run_test() {
    run_test_ipsec_crypto 1500 ipv6 transport tcp trusted_vfs
    run_test_ipsec_crypto 9000 ipv6 transport tcp trusted_vfs
}
