...
function test_miss_rules() {
    ...
    count0_x=`grep -B5 "dst_esw_owner_vhca_id\s*:0x1" /tmp/port0 | grep "source_sqn" | wc -l`
    ((count0-=count0_x))
    ...
    count1_x=`grep -B5 "dst_esw_owner_vhca_id_valid\s*:0x1" /tmp/port1 | grep "source_sqn" | wc -l`
    ((count1-=count1_x))
