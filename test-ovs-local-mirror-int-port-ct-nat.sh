...
#
#
# Test ovs setting with internal port with local mirroring and nat
#
...
function config_ovs() {
    ...
    ovs-ofctl add-flow br-int "table=200,priority=100,in_port=$REP,ip,tcp,ct_state=+new+trk,actions=ct(commit,zone=201,nat(dst=$REMOTE:$REAL_PORT)),normal"
...
function run() {
    ...
    title "test traffic"
    t=15
    on_remote timeout $((t+2)) iperf3 -s -D -p $REAL_PORT
    sleep 1
    ip netns exec ns0 timeout $((t+2)) iperf3 -c $REMOTE -p $FAKE_PORT -t $t -P3 &
    pid2=$!
    ...
    timeout $((t-4)) ip netns exec ns0 tcpdump -qnnei $VF -c 60 port $FAKE_PORT &
    tpid1=$!
