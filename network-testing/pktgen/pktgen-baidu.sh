...
for ((thread = $F_THREAD; thread <= $L_THREAD; thread++)); do
    ...
    pg_set $dev "src_min 198.0.0.0"
    pg_set $dev "src_max 198.$ip_b.$ip_c.$ip_d"
    ...
    pg_set $dev "udp_src_min 9"
    pg_set $dev "udp_src_max 9"
    #5516

    pg_set $dev "flag UDPDST_RND"
    pg_set $dev "udp_dst_min 9"
    pg_set $dev "udp_dst_max 9"
    #5502
